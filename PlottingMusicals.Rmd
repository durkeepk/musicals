...
I'll use some data I found about musicals because I was recently reminded of my childhood love for musicals (especially Rodgers & Hammerstein) and I just received my first desk rejection from Psychological Science (#rightofpassage?), so I don't feel like doing anything even remotely serious.
...
Anyhow, for each musical I typed "[musical name] tracklisting" and "[musical name] runtime" into google and recorded what came up for each search in my spreadsheet, which can be downloaded and contributed to [HERE]). There's probably a way I could have written some code to automatically scrape that data and more, but that would've been a whole thing.
